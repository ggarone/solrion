...
public final class ClassicFacetParamsEmitter
        implements ClassicalFacetVisitor<ParamBag, ParamBag> {
    ...
    public void emit(ClassicFacetsSpec spec, ParamBag p) {
        if (spec == null || !spec.enabled() || p == null) return;
    ...
    public ParamBag visitTerms(TermsFacet f, ParamBag p) {
        ...
        if (f.limit() != null) p.add(base + "limit", f.limit().toString());
        if (f.minCount() != null) p.add(base + "mincount", f.minCount().toString());
        if (f.sort() != null) p.add(base + "sort", f.sort().value());
    ...
    public ParamBag visitRange(RangeFacet<?> f, ParamBag p) {
        ...
        if (f.include() != null) p.add(base + "include", f.include().value());
        if (f.other() != null) p.add(base + "other", f.other().value());
