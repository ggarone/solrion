 ...
 *
 * <p>- Parentheses are applied based on operator precedence to preserve semantics. - Empty
 * expressions render to "" (caller decides whether to omit or default).
 */
public final class SolrDialectTranslator
    implements ExprVisitor<String, SolrDialectTranslator.Context> {
  ...
  private String maybeWrap(int myPrec, Context ctx, String rendered) {
    if (Validate.isBlank(rendered)) {
      return "";
    }
    if (ctx.parentPrec() > myPrec) {
      return "(" + rendered + ")";
    }
    return rendered;
  }
  ...
  public String visitField(Expr.Field field, Context ctx) {
    ...
    if (Validate.isBlank(fn) || Validate.isBlank(v)) {
      return "";
    }
  ...
  public String visitAnd(Expr.And and, Context ctx) {
    ...
    if (Validate.isBlank(left) || Validate.isBlank(right)) {
      return "";
    }
  ...
  public String visitRangeValue(Expr.RangeValue<?> value, Context ctx) {
    ...
    String loV =
        (lo == null || !lo.bounded()) ? "*" : SolrEscaper.escapeTerm(String.valueOf(lo.value()));
    String hiV =
        (hi == null || !hi.bounded()) ? "*" : SolrEscaper.escapeTerm(String.valueOf(hi.value()));
