...
public class RangeFacet<T extends Comparable<T>> implements ClassicFacet {
  ...
  public RangeFacet(
    ...
    this.minCount =
        minCount != null ? Validate.require(minCount >= 0, "minCount must be >= 0") : null;
  ...
  public static <T extends Comparable<T>> RangeFacet<T> ofField(
      String field, Range<T> range, String gap) {
    return RangeFacet.<T>builder().field(field).range(range).gap(gap).build();
  }

  public static <T extends Comparable<T>> RangeFacet<T> ofField(
      String name, String field, Range<T> range, String gap) {
    return RangeFacet.<T>builder().name(name).field(field).range(range).gap(gap).build();
  }
}
