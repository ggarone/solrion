 ...
 *
 * <p>Canonical string form (interval notation): [a,b] [a,b) (a,b] (a,b)
 *
 * <p>Unbounded ends are rendered as: -inf for lower, inf for upper
 *
 * <p>Parsing supports: -inf, inf, +inf, *
 */
public final class Range<T extends Comparable<? super T>> {
  ...
  public static <T extends Comparable<? super T>> Range<T> fromString(
      String raw, Function<String, T> valueParser) {
    Validate.notBlank(raw, "range");
    Validate.notNull(valueParser, "valueParser");

    String s = raw.trim();
    if (s.length() < 5) {
      throw new IllegalArgumentException("Invalid range: " + raw);
    }
    ...
    if (!(lowerInclusive || lowerExclusive)) {
      throw new IllegalArgumentException("Invalid lower bracket in: " + raw);
    }
    if (!(upperInclusive || upperExclusive)) {
      throw new IllegalArgumentException("Invalid upper bracket in: " + raw);
    }
  ...
  private static <T extends Comparable<? super T>> Bound<T> parseLowerBound(
      String token, boolean inclusive, Function<String, T> valueParser, String raw) {
    if (isLowerUnboundedToken(token)) {
      return Bound.unbounded();
    }
    if (isTokenUnbounded(token)) {
      throw new IllegalArgumentException("Invalid lower bound token in: " + raw);
    }
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Failed parsing lower bound '" + token + "' in: " + raw, e);
    }
    if (v == null) {
      throw new IllegalArgumentException("Lower bound parser returned null for: " + raw);
    }
  ...
  private static <T extends Comparable<? super T>> Bound<T> parseUpperBound(
      String token, boolean inclusive, Function<String, T> valueParser, String raw) {
    if (isUpperUnboundedToken(token)) {
      return Bound.unbounded();
    }
    if (isTokenUnbounded(token)) {
      throw new IllegalArgumentException("Invalid upper bound token in: " + raw);
    }
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Failed parsing upper bound '" + token + "' in: " + raw, e);
    }
    if (v == null) {
      throw new IllegalArgumentException("Upper bound parser returned null for: " + raw);
    }
    return inclusive ? Bound.inclusive(v) : Bound.exclusive(v);
  }
  ...
  public String toCanonicalString() {
    return (lower.bounded() ? (lower.inclusive() ? '[' : '(') : '(')
        + (lower.bounded() ? String.valueOf(lower.value()) : "-inf")
        + ','
        + (upper.bounded() ? String.valueOf(upper.value()) : "inf")
        + (upper.bounded() ? (upper.inclusive() ? ']' : ')') : ')');
  }
